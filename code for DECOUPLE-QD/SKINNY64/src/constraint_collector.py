...
def run_milp_if_needed():
    ...
    if os.path.exists(mask_path):
        ...
        print(f"[run MILP] mask not found, running gurobi ...")
...
def main():
    ...
    assert data.shape == expected, (
        f"freq mask shape {data.shape} differs SKINNY-{16*SBOX_SIZE} / {ROUNDS} round "
        f"expect: {expected}, please check utils.SBOX_SIZE / NB_ROUNDS。"
    )
    data = data.tolist()
