...
import importlib
import argparse
my_sbox = [1, 10, 4, 12, 6, 15, 3, 9, 2, 13, 11, 7, 5, 0, 8, 14]
...
parser = argparse.ArgumentParser(description="GIFT-128 SAT Solver")
...
try:
    ...
    print(f"error loading {args.module}")
    exit(1)
except AttributeError:
    print(f"error loading {args.module}'s dic_cons")
    exit(1)
...
if __name__ == "__main__":
    ...
    for i in range(len(dic_cons)):
        ...
        cons_t = cons_pair[0]
        z = cons_pair[1]
        solu_txt = f"solve_results_gift128_{name}.txt"   
