...
def plot_quasi_distribut(trails, appendix='',avg_p=MIN_CORR, num_samples=2000):
    ...
    if total_space <= num_samples:
        ...
        print(f"total space: ({total_space}) too big, sampling......")
    ...
    for i in dist_lst:
        if(i>0):
            cnt_i+=1
    print(len(dist_lst),'/',cnt_i)
    print(nz_lst)
    if(neg_flg):
        print("there is negative value")
    np.save(f'./quasi_distri/distri_{NB_ROUNDS}R{NAME}',dist_lst)
