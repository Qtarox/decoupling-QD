"""
MASK_DIVIDER_TK3.py  --  SKINNY-64 TK3 版本
========================================================

针对 SKINNY-64-TK3 (related-tweakey, 三条 tweakey schedule):
    CELL_SIZE        = 4
...
与 TK1/TK2 的唯一区别是密钥位宽 (每个 cell 由 TK1⊕TK2⊕TK3 注入),
具体差异通过 Global_mat_bit 矩阵的列数自动体现, 本文件循环不动.

约束输出示例:
    + [y_0_0] + x_1_0 + k_0 + k_128 + k_256 = 0   # TK1 + TK2 + TK3 三组 key
...
# TK3 模式自检
if ADV_MODEL != "TK3":
    print(f"[warn] MASK_DIVIDER_TK3 被加载, 但 utils.ADV_MODEL = {ADV_MODEL}")

if len(Sbox) != SBOX_DOMAIN:
    raise RuntimeError(
        f"[MASK_DIVIDER_TK3] SKINNY-64 期望 Sbox 长度 {SBOX_DOMAIN}, "
...
def extract_hidden_constraints(L_original, active_bit_dic, masked_bit_dic, ROUNDS):
    ...
    TK3 下 key 列数最大 (TK1+TK2+TK3), 但本函数通过
    np.shape(L_original)[1] 自动适应.
    """
    ...
    for j in range(STATE_COLS, TOTAL_COLS):
        elim_cols.append(j)   # 所有 key 列 (TK1 + TK2 + TK3) 都消去
    ...
    print(f"[*] GF(2) 高斯消元 (TK3): 试图消去 {elim_count} 个冗余变量 ...")
...
def creat_dic_GIFT(rd):
    ...
    x_dic, y_dic = {}, {}
    skipped = []
    print("running creat_dic_GIFT (SKINNY-64 TK3)!")
    ...
    if skipped:
        print(f"[warn] creat_dic_GIFT 跳过了 {len(skipped)} 个不合法 cell:")
...
def generate_constraints(L_mat, dic_x, active_bit_dic, masked_bit_dic, ROUNDS, L_original):
    ...
    print(f"[generate_constraints/TK3] STATE_COLS={STATE_COLS}, "
    ...
    # 4) 收集未知数 (masked state bits + 涉及的所有 key bits, 包括 TK1+TK2+TK3)
    unknown_vars = set()
    row_to_unknowns = {}
...
if __name__ == "__main__":
    ...
    out_file = f"./constraints/CONS_TK3_{NB_ROUNDS}R_{MIN_CORR}_TH{THRESH}.txt"
