...
if ADV_MODEL != "TK3":
    print(f"[warn] MASK_DIVIDER_TK3 loaded, but utils.ADV_MODEL = {ADV_MODEL}")
if len(Sbox) != SBOX_DOMAIN:
    raise RuntimeError(
        f"warn: GEN_LINEAR.Sbox length: {len(Sbox)}."
    )
...
def load_masks_from_file(file_path, round_num):
    ...
    if not os.path.exists(file_path):
        print(f"erroe: no such {file_path} ")
        return None
    try:
...
def creat_dic_GIFT(rd):
    ...
    if skipped:
        ...
        if len(skipped) > 20:
            print(f"       skipping {len(skipped)} cells")
    return x_dic, y_dic
