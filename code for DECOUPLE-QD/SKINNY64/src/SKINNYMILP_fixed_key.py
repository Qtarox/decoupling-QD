...
def SKINNY_MILP_Quasi_Diff(nb_rounds,current_corr):
    ...
    model.addConstr(quicksum(Q[r, i, j, corr] * corr for r in range(NB_ROUNDS) for i in STATE_RANGE for j in STATE_RANGE for corr in CORR_RANGE) >= -current_corr)
    model.addConstr(quicksum(Q[r, i, j, corr] * corr for r in range(NB_ROUNDS) for i in STATE_RANGE for j in STATE_RANGE for corr in CORR_RANGE) <= -current_corr+0.999)
    ...
    # 3. 如果只是为了看总数，尝试将 MIPFocus 设置为 3 (专注于界限，有时能缩小搜索树)
    model.params.MIPFocus = 3 
    
    # 4. 优化内存的算法选择：使用并发/确定性算法
    model.params.Method = 3 
    
    # 5. 保持你的池搜索模式，但注意如果超过 PoolSolutions，它会丢弃最差的解
    model.params.PoolSearchMode = 2
    model.params.PoolSolutions = 2000000
    ...
    print("solution number:",model.SolCount)
    ...
    # file_mask = open(outputfile_name + "_mask", "x")
...
if __name__=="__main__":
    cnt_lst=[]
    for corr in range(61,62):
        nb_sol = SKINNY_MILP_Quasi_Diff(NB_ROUNDS,corr)
        cnt_lst.append(f'{corr} : {nb_sol}')

    print(cnt_lst)
# ['52.0 : 64',  '53.0 : 256',  '54.0 : 256',  '55.0 : 960',  '56.0 : 7040',  '57.0 : 23040',  '58.0 : 52224','59 : 122624','60 : 349952']